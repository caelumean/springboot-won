...
docker build \
  -t caelumean/spring-board:1.0.0 \
  -t caelumean/spring-board:1.0 \
  -t caelumean/spring-board:1 \
  -t caelumean/spring-board:latest .

# 11. Docker Hub 인증 로그인
docker login

# 12. 빌드된 다중 태그 이미지를 Docker Hub 원격 레지스트리에 일괄 업로드
docker push --all-tags caelumean/spring-board

# 13. Docker Hub 원격 레지스트리 다운로드 연동 테스트를 위해 로컬 빌드 이미지 완전 삭제
docker rmi caelumean/spring-board:1.0.0 caelumean/spring-board:1.0 caelumean/spring-board:1 caelumean/spring-board:latest
